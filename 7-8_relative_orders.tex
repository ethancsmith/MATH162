...
\ifdefined\SOLUTION
\SOLUTION{
Thinking about which term we expect to dominant in both numerator and denominator, we guess that
\begin{equation*}
\frac{x+\tanh x}{\sqrt x + \sin x}\asymp \frac{x}{\sqrt x} = \sqrt x
\end{equation*}
as $x\to\infty$.
We then verify that our guess is correct by computing
\begin{equation*}
\lim_{x\to\infty}\frac{x+\tanh x}{\sqrt x + \sin x}\cdot\frac{1}{\sqrt x} 
=\lim_{x\to\infty}\frac{x\left(1+\frac{\tanh x}{x}\right)}{\sqrt x\left(1 + \frac{\sin x}{\sqrt x}\right)}\cdot\frac{1}{\sqrt x} 
=\lim_{x\to\infty}\frac{1+\frac{\tanh x}{x}}{1 + \frac{\sin x}{\sqrt x}}
=\frac{1+0}{1+0} = 1.
\end{equation*}
}
\fi
